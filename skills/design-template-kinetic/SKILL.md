---
name: design-template-kinetic
description: "Kinetic Typography — viewport typography, infinite marquees, acid yellow #DFE104, massive scale, 0px brutalist. Use when user wants kinetic, marquee, brutalist, typography poster, energetic, motion, acid yellow."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-kinetic

> Kinetic Typography — poster come to life. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-kinetic design-template-<nama>`.
...
- Load `design-template-kinetic` when task is kinetic / marquee / massive type.
- Role = how you work, Design-system = what you build.
- Signature: `clamp(3rem,12vw,14rem)` + marquees 80/40 + acid yellow `#DFE104` + 0px brutalist.

## Kinetic Checklist

Run before delivering. All must be **yes**:

- [ ] Palette `#09090B` / `#FAFAFA` + muted `#27272A`/`#A1A1AA` + accent `#DFE104` black text, border `#3F3F46`. No pure B/W.
- [ ] Typography Space Grotesk ALL UPPERCASE, hero `clamp(3rem,12vw,14rem)` `leading-[0.8]`, body `lg:text-2xl`, massive numbers `8-12rem`.
- [ ] Radius `0px` (or `rounded-sm` max), borders `border-2` `#3F3F46`, no shadows — flat brutalist.
- [ ] Motion: marquees `react-fast-marquee` speed 80 (stats) + 40 (testimonials) `autoFill` no gradient + hero parallax `useScroll` scale 1.2 + sticky `top-32` cards.
- [ ] Interactions: buttons `h-14/20` `scale-105/95`, cards flood `#DFE104` `duration-300` + invert text, inputs `h-24` `border-b-2` `text-4xl`.
- [ ] Layout `max-w-[95vw]` + `py-32`, `gap-px` hairline grids, full bleed marquees, 1→2→3 cols.
- [ ] Bold 7: viewport type + 2 marquees + massive `8-12rem` numbers + hard inversions + aggressive 8-10x scale + uppercase lockup.
- [ ] Accessibility 15:1 AAA, `prefers-reduced-motion` fallback, 44px targets, noise SVG title.
