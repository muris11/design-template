---
name: design-template-cyberpunk
description: "Cyberpunk Glitch — void #0a0a0f, neon green #00ff88, magenta #ff00ff, cyan #00d4ff, Orbitron + JetBrains Mono, chamfer, scanlines, glitch. Use when user wants cyberpunk, glitch, neon, matrix, blade runner, hacker, dystopia, HUD."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-cyberpunk

> Cyberpunk / Glitch — High-Tech Low-Life neon dystopia. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-cyberpunk design-template-<nama>`.
...
- Load `design-template-cyberpunk` when task is cyberpunk / glitch / neon dystopia.
- Role = how you work, Design-system = what you build.
- Signature: Void `#0a0a0f` + Neon `#00ff88`/`#ff00ff`/`#00d4ff` + Orbitron + chamfer clip-path + scanlines + chromatic glitch.

## Cyberpunk Checklist

Run before delivering. All must be **yes**:

- [ ] Palette dark mandatory `#0a0a0f` + `#12121a` + `#1c1c2e` + neon `#00ff88`/`#ff00ff`/`#00d4ff` + `#2a2a3a`, no light mode.
- [ ] Typography Orbitron/S. Tech Mono + JetBrains Mono, H1 `6xl→8xl` uppercase `tracking-widest`, mono labels.
- [ ] Radius `0px` / `2/4px` or `clip-path` chamfer 10px cuts, borders `1/2px` + glow, no rounded.
- [ ] Effects neon `0 0 5px #00ff88` + `box-shadow-neon-lg` + chromatic `text-shadow -1px #ff00ff`, scanlines `repeating-linear` 2/4px, grid 50px 0.03, noise.
- [ ] Components: buttons monospace uppercase chamfer → fill + glow, cards `#12121a` chamfer + terminal/holographic variants, inputs `> ` prefix + `cyber-chamfer-sm` + neon focus.
- [ ] Bold 8: glitched H1 + scanline overlay + terminal `> ` + neon glow borders + chamfer corners + blink cursor + circuit bg + typewriter.
- [ ] Motion `150ms cubic-bezier` or `100ms steps(4)`, keyframes blink/glitch/scanline/rgbShift, sharp digital.
- [ ] Icons lucide `stroke-1.5` `h-5/6` `drop-shadow(0 0 4px currentColor)` in bordered squares.
- [ ] Accessibility 7.5:1 AA, `ring-accent` + glow, `prefers-reduced-motion` disables glitch.
