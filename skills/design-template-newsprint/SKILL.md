---
name: design-template-newsprint
description: "Newsprint design system — golden age journalism, off-white paper #F9F9F7, ink black #111111, editorial red #CC0000, sharp 0px, high density. Use when user wants newsprint, newspaper, editorial, journal, publication, press, broadsheet."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-newsprint

> Newsprint — golden age journalism reimagined. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-newsprint design-template-<nama>`.
...
- Load together with `design-template` core whenever task is newsprint / newspaper.
- Role = how you work, Design-system = what you build.
- Never use soft shadows, blur, or rounded — use `0px` + hard `4px 4px` shadow only.

## Newsprint Checklist

Run before delivering. All must be **yes**:

- [ ] Palette strictly `#F9F9F7` paper / `#111111` ink / `#E5E5E0` divider / `#CC0000` red sparingly. No gradients.
- [ ] Typography Playfair Display (display) + Lora (body) + Inter (UI) + JetBrains Mono (data). Hero `text-5xl→9xl` `leading-[0.9]`.
- [ ] Radius `0px` everywhere via `.sharp-corners`. Borders `1px` standard, `4px` heavy dividers, collapsed grids.
- [ ] Textures: 4×4 dot grid bg + `.newsprint-texture` 3px line grid + radial halftone + ✧ ornaments.
- [ ] Components: buttons invert black↔white, cards `border border-[#111111]`, inputs `border-b-2`, icons `lucide` in bordered boxes.
- [ ] Bold 9: vertical `border-r` columns, drop caps `text-7xl float-left`, marquee ticker, Vol.1 edition metadata, `text-justify`, grayscale→sepia, asymmetric 8/4 splits, `uppercase tracking-widest`, inverted black section.
- [ ] Layout `max-w-screen-xl` 12-col `col-span-*` with collapsed borders, high density `py-16` / `p-6`.
- [ ] Motion snappy `duration-200 ease-out`, hard `4px 4px` shadow + `translate(-2px,-2px)`, no floating. Accordion `grid-rows-[0fr]/[1fr]`.
- [ ] Accessibility AAA 17:1 black/paper, focus `ring-2 ring-neutral-950`, semantic HTML, 44px touch targets.
