---
name: design-template-bauhaus
description: "Bauhaus design system — constructivist geometric modernism with primary colors, hard shadows, thick borders. Use when user wants bauhaus, constructivist, geometric, primary colors, modernist, 1920s poster, form follows function."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-bauhaus

> Bauhaus — constructivist geometric modernism. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-bauhaus design-template-<nama>`.
...
- Load together with `design-template` core whenever task is bauhaus UI.
- Role section is the **how you work** (audit stack → ask scope → plan → code).
- Design-system section is the **what you build** (tokens, components, motion).
- Never mix tokens from other templates. Bauhaus = `Outfit` + primaries `#D02020`/`#1040C0`/`#F0C020` + `#121212`/`#F0F0F0` + hard shadows + border-4.

## Bauhaus Checklist

Run alongside delivery. All must be **yes**:

- [ ] Colors strictly `#F0F0F0` / `#121212` + primaries `#D02020` `#1040C0` `#F0C020` (+ `#E0E0E0` muted). No gradients.
- [ ] Typography Outfit 400/500/700/900, uppercase headlines font-black tracking-tighter leading-[0.9].
- [ ] Radius binary only: `rounded-none` or `rounded-full`. No `rounded-md/lg`.
- [ ] Borders `border-2` (mobile) / `border-4` (desktop) always `#121212`.
- [ ] Shadows hard offset only `shadow-[4px_4px_0px_0px_black]` / `shadow-[8px_8px_0px_0px_black]`, no blur.
- [ ] Buttons: primary red / secondary blue / yellow / outline — all with hard shadow + active press effect.
- [ ] Cards: `bg-white border-4 shadow-[8px]` + geometric corner decoration (circle/square/triangle rotating primaries).
- [ ] Color blocking sections: hero blue, stats yellow, blog blue, benefits red, CTA yellow, footer `#121212`.
- [ ] Geometric compositions: overlapping circle/square/triangle + rotated 45° elements + dot grid pattern.
- [ ] Icons lucide Circle/Square/Triangle/Check/Quote/ArrowRight/ChevronDown in geometric containers.
- [ ] Images grayscale default → `hover:grayscale-0`, avatars `rounded-full`.
- [ ] Animation mechanical snappy `duration-200/300 ease-out`, no soft organic.
- [ ] Responsive: `max-w-7xl`, grids 1→2→4 with `divide-y/x`, border/shadow scaling.
