---
name: design-template-swiss-minimalist
description: "Swiss International — objective grid, Inter grotesque, pure B/W + Swiss Red #FF3000, 0px, pattern texture. Use when user wants swiss, international typographic, grid, objektiv, helvetica, brutal precise."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-swiss-minimalist

> Swiss International (International Typographic Style) — objective grid law. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-swiss-minimalist design-template-<nama>`.
...
- Load `design-template-swiss-minimalist` when task is Swiss International / grid law.
- Role = how you work, Design-system = what you build.
- Signature: `#FFFFFF` + `#000000` + Swiss Red `#FF3000`, Inter 900 UPPERCASE, `0px`, `border-4`, 4 patterns.

## Swiss Minimalist Checklist

Run before delivering. All must be **yes**:

- [ ] Palette strictly `#FFFFFF` / `#000000` / `#F2F2F2` + `#FF3000` red only as signal. No gradients.
- [ ] Typography Inter 900 UPPERCASE `text-6xl→[10rem]` `tracking-tighter/widest`, flush-left ragged-right. Body 400/500.
- [ ] Radius `0px` everywhere, borders `border-2/4` black visible grid, asymmetric 8:4/7:5.
- [ ] Textures 4 patterns: grid 24px 3%, dots 16px 4%, diagonal 45° 2%, noise SVG 1.5% — on muted, not on black/red.
- [ ] Components: buttons `rounded-none` invert Black→Red hover, cards `border-black` bg white/muted `p-8/12` → Red/Black inversion on hover, inputs `border-b` → red focus.
- [ ] Bold: numbered labels `01. System` red, Bauhaus geometric compositions (2×2 grid), full inversions, plus rotate 90°, vertical slide nav.
- [ ] Motion snappy `duration-150/200 ease-out`, no spring. Micro: slide nav, scale 1.05, rotate 90°, lift -1px.
- [ ] Responsive: `text-6xl→[10rem]`, `border-4` never thins, `h-16` CTAs mobile, 1→2→4 cols.
- [ ] Accessibility 21:1, `focus-visible:ring-2 ring-[#FF3000]`, 44px targets, semantic HTML.
