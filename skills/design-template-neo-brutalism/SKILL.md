---
name: design-template-neo-brutalism
description: "Neo-brutalism — cream #FFFDF5, pure black #000, hot red #FF6B6B, yellow #FFD93D, violet #C4B5FD, hard 0px + 4px borders + solid shadows. Use when user wants neo brutalism, neu brutalism, sticker, punk, memphis, y2k, hard shadow."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-neo-brutalism

> Neo-brutalism (Neu-Brutalism) — digital punk rebellion. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-neo-brutalism design-template-<nama>`.
...
- Load `design-template-neo-brutalism` when task is Neo-brutalism / sticker punk.
- Role = how you work, Design-system = what you build.
- Signature: `#FFFDF5` cream + `#000` + `#FF6B6B`/`#FFD93D`/`#C4B5FD` + Space Grotesk 900 + `border-4` + `shadow-[8px]` solid.

## Neo-brutalism Checklist

Run before delivering. All must be **yes**:

- [ ] Palette cream `#FFFDF5` + black `#000` + red `#FF6B6B` + yellow `#FFD93D` + violet `#C4B5FD`, no grays. Cream canvas, black structure.
- [ ] Typography Space Grotesk 900 `text-8xl/9xl` uppercase `tracking-tighter`, body 700 `text-lg/xl`, stroke `-webkit-text-stroke:2px black` for display.
- [ ] Radius `0px` default (sharp) or `rounded-full` pills only. Borders `border-4` black mandatory everywhere.
- [ ] Shadows hard solid `shadow-[8px_8px_0px_0px_#000]` zero blur, bottom-right 45°, text-shadow 4px black. No blur/backdrop.
- [ ] Textures halftone `radial-gradient 1.5px 20px`, grid `40px`, noise SVG, radial dots `30px` — on every flat bg.
- [ ] Components: buttons `h-12/14` `border-4` `shadow-[4px]` + `active:translate 2px shadow-none` push, cards `bg-white border-4 shadow-[8px] hover:-translate-y-2`, inputs `border-4` focus yellow.
- [ ] Bold: text-stroke hollow + sticker rotate `1/-2/3°` + push/lift/rotate physics + stars/Arrows + border everywhere + halftone overlays + asymmetric 60/40.
- [ ] Motion mechanical `duration-100/200 ease-linear`, `animate-spin-slow 10s`, no `ease-in-out` slow.
- [ ] Layout `max-w-7xl` `py-16→32` `gap-8→12`, marquee `react-fast-marquee`, overlap absolute, chaos zones stacked shapes.
- [ ] Accessibility AAA on cream/black, `ring-black` focus, 44px `h-12/14`, semantic HTML, `aria-label` icons.
