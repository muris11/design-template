---
name: design-template-academia
description: "Academia Classical — mahogany #1C1714, parchment #E8DFD4, brass #C9A962, crimson #8B2635, Cormorant + Crimson Pro + Cinzel, arch-top. Use when user wants academia, classical, scholarly, library, university, victorian, manuscript, parchment, brass."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-academia

> Academia / Classical — scholarly library at night. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-academia design-template-<nama>`.
...
- Load `design-template-academia` when task is academia / library / classical.
- Role = how you work, Design-system = what you build.
- Signature: Mahogany `#1C1714` + Parchment `#E8DFD4` + Brass `#C9A962` + Crimson `#8B2635` + arch-top.

## Academia Checklist

Run before delivering. All must be **yes**:

- [ ] Palette mahogany `#1C1714` + oak `#251E19` + parchment `#E8DFD4` + brass `#C9A962` + crimson `#8B2635`, borders `#4A3F35`.
- [ ] Typography Cormorant (head) + Crimson Pro (body) + Cinzel (display) + drop caps `7xl` float-left + Volume I Roman numerals.
- [ ] Radius `4px` default + arch `40% 40% 0 0 / 20% 20% 0 0`, borders `1px` wood / `2px` brass.
- [ ] Effects sepia `0.6` → `0` `700ms`, brass gradient, engraved `text-shadow`, paper noise `0.03` + vignette overlay.
- [ ] Components: buttons `h-12` Cinzel brass gradient / crimson hover, cards `#251E19` flourishes 24px, inputs `h-12` `#251E19`.
- [ ] Bold 10: arch-top + sepia→color + Volume I + drop caps + flourishes 40px/24px + ornate dividers ✶ + wax seal crimson + brass interactivity + engraved + textures.
- [ ] Motion dignified `150/300/500/700ms` `ease-out`, no bounce.
- [ ] Layout `py-24→32` `max-w-6xl` Volume system, generous breathing.
- [ ] Accessibility 8.5:1 AAA, `ring-[#C9A962]` brass, semantic HTML.
