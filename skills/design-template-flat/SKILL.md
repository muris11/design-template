---
name: design-template-flat
description: "Flat Design — zero depth, pure white #FFFFFF, blue #3B82F6, emerald #10B981, amber #F59E0B, Outfit, color blocks. Use when user wants flat design, flat, color block, poster, zero shadow, bold reduction."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-flat

> Flat Design — confident reduction, zero Z-axis. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-flat design-template-<nama>`.
...
- Load `design-template-flat` when task is flat / color block / poster.
- Role = how you work, Design-system = what you build.
- Signature: `shadow-none` + `#FFFFFF` + Blue #3B82F6 / Emerald #10B981 / Amber #F59E0B + Outfit + `rounded-md/lg`.

## Flat Checklist

Run before delivering. All must be **yes**:

- [ ] Palette `#FFFFFF` + `#111827` + `#3B82F6` / `#10B981` / `#F59E0B` + `#F3F4F6`/`#E5E7EB`. No muddy pastels.
- [ ] Typography Outfit 700/800 tight `-0.02em`, body 400, labels 500 uppercase `tracking-wider`.
- [ ] Radius `rounded-md/lg` (6-8px) consistent, pill only for tags. Borders `0px` except `border-2/4` where needed.
- [ ] Shadows `shadow-none` absolute. No blur/backdrop. Background deco subtle `bg-white/5` geometric only.
- [ ] Components: buttons `h-14/16` `hover:scale-105` color shift no shadow, cards color-block `p-6/8` `hover:scale-[1.02]` `hover:bg-*-100`, inputs `bg-gray-100` → focus `border-2` blue + white bg.
- [ ] Sections alternating White/Gray/Bold accent blocks, sharp transitions, no thin dividers (except FAQ `border-2`).
- [ ] Icons lucide 2-2.5px in `h-14/16` white circles `bg-white text-blue-600` + `group-hover:scale-110`.
- [ ] Motion snappy `duration-200/300` scale + color fill only, no shadow depth.
- [ ] Bold: large geometric shapes, full-section color blocks, pricing scale, multi-color stats, thick `border-4` outlines.
