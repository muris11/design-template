---
name: design-template-art-deco
description: "Art Deco Gatsby — obsidian #0A0A0A, champagne #F2F0E4, gold #D4AF37, Marcellus + Josefin Sans, stepped corners, sunburst. Use when user wants art deco, gatsby, roaring twenties, metropolis, zigzag, sunburst, geometric luxury."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-art-deco

> Art Deco (The Gatsby Aesthetic) — Roaring Twenties opulence. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-art-deco design-template-<nama>`.
...
- Load `design-template-art-deco` when task is Art Deco / Gatsby / 1920s luxury.
- Role = how you work, Design-system = what you build.
- Signature: Obsidian `#0A0A0A` + Champagne `#F2F0E4` + Gold `#D4AF37` + Marcellus/Josefin Sans, `0px`, stepped corners, sunburst, rotated diamonds.

## Art Deco Checklist

Run before delivering. All must be **yes**:

- [ ] Palette dark luxury `#0A0A0A` + `#141414` + `#F2F0E4` + Gold `#D4AF37` + `#1E3D59` muted, borders gold celebrated.
- [ ] Typography Marcellus 6xl/7xl uppercase `tracking-widest` + Josefin Sans body `text-lg`, 0px / 2px, stepped corners via clip-path.
- [ ] Effects gold glow `0 0 15px rgba(212,175,55,0.2)` + metallic gradient + grain noise, no drop shadows.
- [ ] Components: buttons `h-12` `rounded-none` caps wide tracking 2px gold border → gold bg, cards `#141414` `1px` gold 30%→100% + L-brackets + `-translate-y-2`, inputs `border-b-2` gold `h-12`.
- [ ] Bold 10: diagonal crosshatch 3-5% + rotated diamonds `rotate-45` + roman numerals I II III + L-brackets + double-frame images grayscale + sunburst `radial-gradient` + `h-px w-24` dividers + `w-px` verticals + glow + caps `tracking-widest`.
- [ ] Layout `max-w-6xl/7xl` `py-32` `gap-8` centered axis, symmetry ceremonial, even columns 3/2/1.
- [ ] Motion theatrical `duration-300/500 ease-out`, `-translate-y-2`, `scale-105`, chevron `rotate-180`, no bounce.
- [ ] Accessibility gold/black 7:1 AA, `ring-[#D4AF37]`, 48px `h-12`, `aria-hidden` decor, 44px min.
