---
name: design-template-luxury
description: "Luxury Editorial — warm alabaster #F9F8F6, charcoal #1A1A1A, gold #D4AF37, Playfair Display, 0px, cinematic slow. Use when user wants luxury, editorial, vogue, chanel, hermes, kinfolk, expensive, tactile, fashion magazine."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-luxury

> Luxury / Editorial — expensive paper tactile. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-luxury design-template-<nama>`.
...
- Load `design-template-luxury` when task is luxury / editorial / expensive paper.
- Role = how you work, Design-system = what you build.
- Signature: Alabaster `#F9F8F6` + Charcoal `#1A1A1A` + Gold `#D4AF37`, Playfair/Inter, 0px, cinematic 1500-2000ms.

## Luxury Checklist

Run before delivering. All must be **yes**:

- [ ] Palette warm alabaster/charcoal, not pure B/W. Gold only as accent.
- [ ] Typography Playfair massive `6xl→9xl` + Inter body + wide tracking `0.25em` labels, tight `0.9` headlines.
- [ ] All `0px`, `1px` borders `#1A1A1A`/`/10-20`, shadows subtle layered `0_4px_24px rgba(0,0,0,0.08)`.
- [ ] Images `grayscale` → `grayscale-0` `1500-2000ms` + `scale-105`, paper noise 2%.
- [ ] Grid 4 vertical lines `w-px bg-[#1A1A1A]/20`, asymmetric 7/5, `max-w-[1600px]`, `py-24→32`.
- [ ] Components: buttons `h-12` sliding gold overlay + `translate-x`, cards `border-t` only, inputs `border-b` italic placeholder.
- [ ] Bold 10: vertical labels `writing-mode`, drop caps `7xl float-left`, mixed italic headlines gold, grid lines, gold slide, `h-px w-8` decor, layered shadows.
- [ ] Motion slow `500→2000ms` `cubic-bezier(0.25,0.46,0.45,0.94)`, no snap.
- [ ] Accessibility 12.6:1 AAA, `focus-visible` elegant, 48px targets.
