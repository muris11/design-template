---
name: design-template-bold-typography
description: "Bold Typography — dark #0A0A0A + warm white #FAFAFA + vermillion #FF3D00, Inter Tight + Playfair Display, 0px, 6:1 scale. Use when user wants bold typography, poster, editorial, type hero, extreme scale, underline."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-bold-typography

> Bold Typography — poster design translated to web. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-bold-typography design-template-<nama>`.
...
- Load `design-template-bold-typography` when task is bold typographic / poster.
- Role = how you work, Design-system = what you build.
- Signature: `#0A0A0A` + `#FAFAFA` + vermillion `#FF3D00` + Inter Tight / Playfair, `0px`, underline.

## Bold Typography Checklist

Run before delivering. All must be **yes**:

- [ ] Palette dark `#0A0A0A` + `#FAFAFA` + muted `#1A1A1A`/`#737373` + vermillion `#FF3D00`, border `#262626`. Restrained 1 accent.
- [ ] Typography Inter Tight headlines `5xl→8xl` `tracking-tighter/-0.06em` + Playfair quotes + Mono labels, scale 6:1 hero/body, `0px` radius.
- [ ] Components: buttons underline `h-0.5` `scale-x-110` / outline invert, cards `border` `bg-transparent` `p-6/8` no shadow, inputs `bg-[#1A1A1A]` `h-12/14` border accent focus.
- [ ] Layout `max-w-5xl` asymmetric 7/5, `py-20→40`, noise grain 1.5% SVG, layered type depth (accent bars `h-1 w-16`).
- [ ] Motion fast decisive `150/200/500ms` `cubic-bezier(0.25,0,0,1)`, underline scale, fade+slide 500ms stagger 80ms, no bounce.
- [ ] Icons lucide `stroke-1.5` 16-24px `currentColor` sparingly left-of-text.
- [ ] Accessibility 18:1 AAA, `ring-accent` 2px, 44px targets, underline 2px+ visible.
