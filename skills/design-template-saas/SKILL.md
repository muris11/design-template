---
name: design-template-saas
description: "Minimalist Modern SaaS — Electric Blue gradient #0052FF→#4D7CFF, Calistoga + Inter, premium depth. Use when user wants saas, minimalist modern, electric blue, agency, premium saas, tech startup."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-saas

> Minimalist Modern SaaS — Electric Blue signature. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-saas design-template-<nama>`.
...
- Load `design-template-saas` when task is SaaS / Minimalist Modern.
- Role = how you work, Design-system = what you build.
- Signature: Electric Blue `#0052FF→#4D7CFF` gradient + Calistoga/Inter + inverted sections.

## SaaS Checklist

Run before delivering. All must be **yes**:

- [ ] Gradient `#0052FF→#4D7CFF` appears on primary CTAs, feature icons, key headline words via `bg-clip-text`.
- [ ] Inverted section exists (`bg-[#0F172A] text-white` + dot pattern) for stats/CTA.
- [ ] Hero has rotating ring (60s) + floating cards (±10px, 4-5s) + accent dot grid.
- [ ] Dual-font: Calistoga headlines, Inter body, JetBrains Mono labels. Tracking/spacing as spec.
- [ ] Cards `rounded-xl/2xl` white + `shadow-md→xl` + gradient border on featured. Spacing `py-28→44`.
- [ ] Micro-motion: pulsing dots, stagger 0.1s, expo-out `[0.16,1,0.3,1]`, `duration-700`.
- [ ] Accessibility: 44px targets, focus `ring-accent`, contrast AA, reduced-motion respected.
