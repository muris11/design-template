---
name: design-template-material
description: "Material You (M3) — seed purple #6750A4, tonal surfaces #FFFBFE, pill buttons, organic blur, Roboto. Use when user wants material you, material design 3, m3, google material, pill, tonal, organic."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-material

> Material You (Material Design 3) — personal tonal adaptive. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-material design-template-<nama>`.
...
- Load `design-template-material` when task is Material You / M3.
- Role = how you work, Design-system = what you build.
- Signature: Seed `#6750A4` + `#FFFBFE` + pill `rounded-full` + `blur-3xl` + state layers.

## Material Checklist

Run before delivering. All must be **yes**:

- [ ] Palette seed `#6750A4` primary + `#FFFBFE` surface + `#F3EDF7` container + `#E8DEF8` secondary, never pure white.
- [ ] Typography Roboto 400/500/700, Display 56px → Label 12px, tight 1.2 for headlines.
- [ ] Radius pill `rounded-full` for ALL buttons/chips, `24px` cards, `48px` hero, inputs `rounded-t-lg` square bottom.
- [ ] Shadows `shadow-sm→md→lg` tonal, no heavy drops, `blur-3xl` organic shapes 10-30% off-canvas.
- [ ] Components: buttons pill `h-10/12` `active:scale-95` state layers `bg/90`, `bg/10`, cards `#F3EDF7` `hover:scale-[1.02]`, inputs `h-14` `border-b-2`.
- [ ] Bold 7: blur shapes layered + tonal depth + pills + `32-48px` radii + state layers `cubic-bezier(0.2,0,0,1)` + elevation `md:-translate-y-4`.
- [ ] Motion `200/300/500ms` `cubic-bezier(0.2,0,0,1)`, `scale-95`, no radius/layout anim.
- [ ] Accessibility 4.5:1 AAA on primary, `ring-md-primary` focus, 44px targets, `aria-hidden` decor.
