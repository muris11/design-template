---
name: design-template-terminal
description: "Terminal CLI — monospace supremacy, phosphor green #33ff00, amber #ffb000, 0px, CRT scanlines, shell metaphors. Use when user wants terminal, cli, hacker, cyber, industrial, shell, monospace, mainframe, code."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-terminal

> Terminal CLI — raw system aesthetic. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-terminal design-template-<nama>`.
...
- Load `design-template-terminal` when task is terminal / CLI / hacker.
- Role = how you work, Design-system = what you build.
- Signature: `#0a0a0a` + `#33ff00` green (+ amber #ffb000), JetBrains Mono, 0px, phosphor glow, `> $ _` prompts.

## Terminal Checklist

Run before delivering. All must be **yes**:

- [ ] Palette dark only `#0a0a0a` bg + `#33ff00` primary + `#ffb000` secondary + `#1f521f` muted/border. No light mode.
- [ ] Typography 100% monospace JetBrains Mono/Fira Code/VT323, headers ALL CAPS, modular scale snapping.
- [ ] Radius `0px` everywhere, borders `1px` solid/dashed green, windows/panes with title bar `+--- ---+`.
- [ ] Effects: `text-shadow: 0 0 5px rgba(51,255,0,0.5)` glow, CRT scanlines overlay, no drop shadows.
- [ ] Components: buttons `[ BRACKETS ]` or inverted block hover (`bg-[#33ff00] text-black`), cards black+green border, inputs `user@acme:~$` + `█` blink block cursor.
- [ ] Layout tmux/vim splits grid, ASCII dividers `----` / `====` / `//`, strict character alignment.
- [ ] Bold: ASCII art logo, typewriter `typing-demo` hero, raw bars `[||||||||||.....]` stats, `animate-blink` cursor, glitch hover.
- [ ] Icons lucide `stroke-2` always `#33ff00`, pixelated/low-fi, no color variants.
- [ ] Motion blink/glitch/typing only, no smooth modern easing.
- [ ] Accessibility phosphor contrast AA, inverted focus visible.
