---
name: design-template-modern-dark
description: "Modern Dark (Linear) design system — deep space ambient lighting, indigo glow, layered gradients, mouse spotlights. Use when user wants modern dark, linear, vercel, raycast, dark premium, ambient glow, cinematic tech, indigo."
allowed-tools: Read Write Edit Glob Grep
---
# design-template-modern-dark

> Modern Dark (Linear) — deep space ambient lighting. One template in `design-template` family. Copy this folder to create next template: `Copy-Item design-template-modern-dark design-template-<nama>`.
...
- Load together with `design-template` core whenever task is modern dark / linear UI.
- Role = how you work, Design-system = what you build.
- Never use pure black #000000 — use #050506 / #020203. Never flat bg — layer 4 bg layers.

## Modern Dark Checklist

Run before delivering. All must be **yes**:

- [ ] Background 4-layer system: base radial gradient + noise (0.015) + 3-4 animated blobs (blur 100-150px) + 64px grid.
- [ ] Palette strictly deep space: `#050506` base, `#020203` deep, `#0a0a0c` elevated, `#5E6AD2` accent, `#EDEDEF` text. No pure black/white.
- [ ] Typography Inter/Geist Sans, gradient text `bg-clip-text`, tight tracking, `text-7xl/8xl` hero.
- [ ] Radius/border: `rounded-2xl` cards, `border-white/[0.06]`, `rounded-lg` buttons, no harsh borders.
- [ ] Shadows 3-4 layers: border highlight + diffuse + accent glow + inner highlight. Accent buttons with glow.
- [ ] Cards with mouse-tracking spotlight (300px radial accent 15%) + top inner glow line.
- [ ] Buttons: primary `#5E6AD2` with glow + active `scale-[0.98]`, secondary `white/[0.05]`.
- [ ] Layout bento asymmetric (col-span mixes, auto-rows 180px), not uniform grid. Section `border-t white/[0.06]`.
- [ ] Animations 200-300ms expo-out `[0.16,1,0.3,1]`, parallax hero (fade/scale/y), stagger 0.08s, blobs float 8-10s.
- [ ] Signature 6 present: blobs + spotlights + gradient type + multi-shadows + parallax + micro-interactions.
- [ ] Accessibility: 15:1 primary, 6:1 muted, focus rings accent, reduced-motion respected.
